            ...
            while (true) {
                ...
                switch (message.type) {
                    ...
                    case "assistant": {
                        ...
                        // fallback.
                        const assistantMessageId = message.type === "assistant" ? message.message.id : undefined;
                        const textAlreadyStreamed = !!assistantMessageId && session.textStreamedMessageIds.has(assistantMessageId);
                        if (message.type === "assistant" &&
                            !textAlreadyStreamed &&
                            Array.isArray(message.message.content) &&
                            message.message.content.some((item) => item.type === "text" || item.type === "thinking")) {
                            this.logger.log(`[claude-agent-acp] No streamed text/thinking deltas seen for ` +
