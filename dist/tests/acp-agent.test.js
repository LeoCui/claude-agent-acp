...
describe("stop reason propagation", () => {
    ...
    function injectSession(agent, messages) {
        ...
        agent.sessions["test-session"] = {
            ...
            taskState: new Map(),
            textStreamedMessageIds: new Set(),
            fallbackEmittedBlockKeys: new Set(),
        };
    }
...
describe("assistant text fallback when streaming yields no text deltas", () => {
    ...
    function makeAssistantMessage(messageId, text) {
    ...
    function makeAssistantMessageWithBlocks(messageId, content) {
        return {
            type: "assistant",
            parent_tool_use_id: null,
            uuid: randomUUID(),
    ...
    it("emits distinct text blocks that share a message id", async () => {
        ...
        const firstChunks = updates.filter((u) => u.sessionUpdate === "agent_message_chunk" &&
            u.content.type === "text" &&
            u.content.text === firstText);
        const secondChunks = updates.filter((u) => u.sessionUpdate === "agent_message_chunk" &&
            u.content.type === "text" &&
    ...
    it("does not duplicate text when an incremental block is followed by a consolidated message", async () => {
        ...
        expect(response.stopReason).toBe("end_turn");
        const updates = notifications.map((n) => n.update);
        const textChunks = updates.filter((u) => u.sessionUpdate === "agent_message_chunk" &&
            u.content.type === "text" &&
            u.content.text === finalText);
        const thoughtChunks = updates.filter((u) => u.sessionUpdate === "agent_thought_chunk" &&
            u.content.type === "text" &&
